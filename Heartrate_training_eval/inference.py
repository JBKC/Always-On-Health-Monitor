...
def inference(dict, s):
    ...
    x, y, act = temporal_pairs(dict, s)

    print(f'Evaluating Session {s}')

    s = int(s[1:])               # session number
    ...
    try:
        checkpoint_path = f'../models/temporal_attention_model_full_augment_session_{s}.pth'
        # checkpoint_path = f'../dummy_models/temporal_attention_model_session_{s}.pth'
